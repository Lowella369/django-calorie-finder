...
class FoodCalorieForm(forms.Form):
 ...
 def search(self):
  ...
  headers = {'x-app-id': settings.APP_ID,'x-app-key': settings.APP_KEY,'x-remote-user-id': '0', 
  ...
  if response.status_code == 200:
   ...
   context = {
    'name': food_data['food_name'],
	  ...
	  'protein_daily_percent': round(food_data['nf_protein'] / Food_PercentValue.rec_dailyValueProtein * 100, 2),}
   return context
  else:
   ...
   if response.status_code == 404:
    ...
    result['message'] = 'API Server is not available at the moment. Try again later.'
  return result
